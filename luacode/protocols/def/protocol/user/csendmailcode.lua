-- Decompiled using luadec 2.2 rev: 895d923 for Lua 5.3 from https://github.com/viruscamp/luadec
-- Command line: -se UTF8 luacode/protocols/def/protocol/user/csendmailcode.lua 

-- params : ...
-- function num : 0 , upvalues : _ENV
local ProtocolBufferStaticFunctions = ((CS.PixelNeko).Net).ProtocolBufferStaticFunctions
local CSendMailCode = dataclass("CSendMailCode", require("framework.net.protocol"))
CSendMailCode.ProtocolType = 2255
CSendMailCode.MaxSize = 65535
CSendMailCode.address = ""
CSendMailCode.Ctor = function(self, client)
  -- function num : 0_0 , upvalues : CSendMailCode
  ((CSendMailCode.super).Ctor)(self, client)
end

CSendMailCode.Marshal = function(self, buffer)
  -- function num : 0_1 , upvalues : ProtocolBufferStaticFunctions
  if not (ProtocolBufferStaticFunctions.WriteProtocolString)(buffer, self.address) then
    return false
  end
  return true
end

CSendMailCode.Unmarshal = function(self, buffer)
  -- function num : 0_2 , upvalues : ProtocolBufferStaticFunctions
  local ret = true
  ret = (ProtocolBufferStaticFunctions.ReadProtocolString)(buffer)
  if not ret then
    return ret
  end
  return ret
end

return CSendMailCode

