-- Decompiled using luadec 2.2 rev: 895d923 for Lua 5.3 from https://github.com/viruscamp/luadec
-- Command line: -se UTF8 luacode/protocols/def/protocol/user/cfinishwitchskillguide.lua 

-- params : ...
-- function num : 0 , upvalues : _ENV
local ProtocolBufferStaticFunctions = ((CS.PixelNeko).Net).ProtocolBufferStaticFunctions
local CFinishWitchSkillGuide = dataclass("CFinishWitchSkillGuide", require("framework.net.protocol"))
CFinishWitchSkillGuide.ProtocolType = 2218
CFinishWitchSkillGuide.MaxSize = 65535
CFinishWitchSkillGuide.skillItemId = 0
CFinishWitchSkillGuide.Ctor = function(self, client)
  -- function num : 0_0 , upvalues : CFinishWitchSkillGuide
  ((CFinishWitchSkillGuide.super).Ctor)(self, client)
end

CFinishWitchSkillGuide.Marshal = function(self, buffer)
  -- function num : 0_1 , upvalues : ProtocolBufferStaticFunctions
  if not (ProtocolBufferStaticFunctions.WriteInt32)(buffer, self.skillItemId) then
    return false
  end
  return true
end

CFinishWitchSkillGuide.Unmarshal = function(self, buffer)
  -- function num : 0_2 , upvalues : ProtocolBufferStaticFunctions
  local ret = true
  ret = (ProtocolBufferStaticFunctions.ReadInt32)(buffer)
  if not ret then
    return ret
  end
  return ret
end

return CFinishWitchSkillGuide

