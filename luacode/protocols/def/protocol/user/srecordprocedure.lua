-- Decompiled using luadec 2.2 rev: 895d923 for Lua 5.3 from https://github.com/viruscamp/luadec
-- Command line: -se UTF8 luacode/protocols/def/protocol/user/srecordprocedure.lua 

-- params : ...
-- function num : 0 , upvalues : _ENV
local ProtocolBufferStaticFunctions = ((CS.PixelNeko).Net).ProtocolBufferStaticFunctions
local SRecordProcedure = dataclass("SRecordProcedure", require("framework.net.protocol"))
SRecordProcedure.ProtocolType = 2216
SRecordProcedure.MaxSize = 65535
SRecordProcedure.procedure = 0
SRecordProcedure.Ctor = function(self, client)
  -- function num : 0_0 , upvalues : SRecordProcedure
  ((SRecordProcedure.super).Ctor)(self, client)
end

SRecordProcedure.Marshal = function(self, buffer)
  -- function num : 0_1 , upvalues : ProtocolBufferStaticFunctions
  if not (ProtocolBufferStaticFunctions.WriteInt32)(buffer, self.procedure) then
    return false
  end
  return true
end

SRecordProcedure.Unmarshal = function(self, buffer)
  -- function num : 0_2 , upvalues : ProtocolBufferStaticFunctions
  local ret = true
  ret = (ProtocolBufferStaticFunctions.ReadInt32)(buffer)
  if not ret then
    return ret
  end
  return ret
end

return SRecordProcedure

