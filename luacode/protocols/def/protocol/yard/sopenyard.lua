-- Decompiled using luadec 2.2 rev: 895d923 for Lua 5.3 from https://github.com/viruscamp/luadec
-- Command line: -se UTF8 luacode/protocols/def/protocol/yard/sopenyard.lua 

-- params : ...
-- function num : 0 , upvalues : _ENV
local ProtocolBufferStaticFunctions = ((CS.PixelNeko).Net).ProtocolBufferStaticFunctions
local SOpenYard = dataclass("SOpenYard", require("framework.net.protocol"))
SOpenYard.ProtocolType = 2351
SOpenYard.MaxSize = 65535
SOpenYard.sceneId = 0
SOpenYard.Ctor = function(self, client)
  -- function num : 0_0 , upvalues : SOpenYard
  ((SOpenYard.super).Ctor)(self, client)
end

SOpenYard.Marshal = function(self, buffer)
  -- function num : 0_1 , upvalues : ProtocolBufferStaticFunctions
  if not (ProtocolBufferStaticFunctions.WriteInt32)(buffer, self.sceneId) then
    return false
  end
  return true
end

SOpenYard.Unmarshal = function(self, buffer)
  -- function num : 0_2 , upvalues : ProtocolBufferStaticFunctions
  local ret = true
  ret = (ProtocolBufferStaticFunctions.ReadInt32)(buffer)
  if not ret then
    return ret
  end
  return ret
end

return SOpenYard

