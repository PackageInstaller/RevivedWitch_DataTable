-- Decompiled using luadec 2.2 rev: 895d923 for Lua 5.3 from https://github.com/viruscamp/luadec
-- Command line: -se UTF8 luacode/protocols/def/protocol/activity/creceivedbpaward.lua 

-- params : ...
-- function num : 0 , upvalues : _ENV
local ProtocolBufferStaticFunctions = ((CS.PixelNeko).Net).ProtocolBufferStaticFunctions
local CReceivedBpAward = dataclass("CReceivedBpAward", require("framework.net.protocol"))
CReceivedBpAward.ProtocolType = 2675
CReceivedBpAward.MaxSize = 65535
CReceivedBpAward.awardId = 0
CReceivedBpAward.Ctor = function(self, client)
  -- function num : 0_0 , upvalues : CReceivedBpAward
  ((CReceivedBpAward.super).Ctor)(self, client)
end

CReceivedBpAward.Marshal = function(self, buffer)
  -- function num : 0_1 , upvalues : ProtocolBufferStaticFunctions
  if not (ProtocolBufferStaticFunctions.WriteInt32)(buffer, self.awardId) then
    return false
  end
  return true
end

CReceivedBpAward.Unmarshal = function(self, buffer)
  -- function num : 0_2 , upvalues : ProtocolBufferStaticFunctions
  local ret = true
  ret = (ProtocolBufferStaticFunctions.ReadInt32)(buffer)
  if not ret then
    return ret
  end
  return ret
end

return CReceivedBpAward

