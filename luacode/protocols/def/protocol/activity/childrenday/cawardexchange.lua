-- Decompiled using luadec 2.2 rev: 895d923 for Lua 5.3 from https://github.com/viruscamp/luadec
-- Command line: -se UTF8 luacode/protocols/def/protocol/activity/childrenday/cawardexchange.lua 

-- params : ...
-- function num : 0 , upvalues : _ENV
local ProtocolBufferStaticFunctions = ((CS.PixelNeko).Net).ProtocolBufferStaticFunctions
local CAwardExchange = dataclass("CAwardExchange", require("framework.net.protocol"))
CAwardExchange.ProtocolType = 2652
CAwardExchange.MaxSize = 65535
CAwardExchange.node = 0
CAwardExchange.Ctor = function(self, client)
  -- function num : 0_0 , upvalues : CAwardExchange
  ((CAwardExchange.super).Ctor)(self, client)
end

CAwardExchange.Marshal = function(self, buffer)
  -- function num : 0_1 , upvalues : ProtocolBufferStaticFunctions
  if not (ProtocolBufferStaticFunctions.WriteInt32)(buffer, self.node) then
    return false
  end
  return true
end

CAwardExchange.Unmarshal = function(self, buffer)
  -- function num : 0_2 , upvalues : ProtocolBufferStaticFunctions
  local ret = true
  ret = (ProtocolBufferStaticFunctions.ReadInt32)(buffer)
  if not ret then
    return ret
  end
  return ret
end

return CAwardExchange

