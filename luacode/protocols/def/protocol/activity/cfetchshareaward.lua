-- Decompiled using luadec 2.2 rev: 895d923 for Lua 5.3 from https://github.com/viruscamp/luadec
-- Command line: -se UTF8 luacode/protocols/def/protocol/activity/cfetchshareaward.lua 

-- params : ...
-- function num : 0 , upvalues : _ENV
local ProtocolBufferStaticFunctions = ((CS.PixelNeko).Net).ProtocolBufferStaticFunctions
local CFetchShareAward = dataclass("CFetchShareAward", require("framework.net.protocol"))
CFetchShareAward.ProtocolType = 2695
CFetchShareAward.MaxSize = 65535
CFetchShareAward.awardId = 0
CFetchShareAward.Ctor = function(self, client)
  -- function num : 0_0 , upvalues : CFetchShareAward
  ((CFetchShareAward.super).Ctor)(self, client)
end

CFetchShareAward.Marshal = function(self, buffer)
  -- function num : 0_1 , upvalues : ProtocolBufferStaticFunctions
  if not (ProtocolBufferStaticFunctions.WriteInt32)(buffer, self.awardId) then
    return false
  end
  return true
end

CFetchShareAward.Unmarshal = function(self, buffer)
  -- function num : 0_2 , upvalues : ProtocolBufferStaticFunctions
  local ret = true
  ret = (ProtocolBufferStaticFunctions.ReadInt32)(buffer)
  if not ret then
    return ret
  end
  return ret
end

return CFetchShareAward

