-- Decompiled using luadec 2.2 rev: 895d923 for Lua 5.3 from https://github.com/viruscamp/luadec
-- Command line: -se UTF8 luacode/protocols/def/protocol/activity/creceivesummerreward.lua 

-- params : ...
-- function num : 0 , upvalues : _ENV
local ProtocolBufferStaticFunctions = ((CS.PixelNeko).Net).ProtocolBufferStaticFunctions
local CReceiveSummerReward = dataclass("CReceiveSummerReward", require("framework.net.protocol"))
CReceiveSummerReward.ProtocolType = 2721
CReceiveSummerReward.MaxSize = 65535
CReceiveSummerReward.rewardId = 0
CReceiveSummerReward.Ctor = function(self, client)
  -- function num : 0_0 , upvalues : CReceiveSummerReward
  ((CReceiveSummerReward.super).Ctor)(self, client)
end

CReceiveSummerReward.Marshal = function(self, buffer)
  -- function num : 0_1 , upvalues : ProtocolBufferStaticFunctions
  if not (ProtocolBufferStaticFunctions.WriteInt32)(buffer, self.rewardId) then
    return false
  end
  return true
end

CReceiveSummerReward.Unmarshal = function(self, buffer)
  -- function num : 0_2 , upvalues : ProtocolBufferStaticFunctions
  local ret = true
  ret = (ProtocolBufferStaticFunctions.ReadInt32)(buffer)
  if not ret then
    return ret
  end
  return ret
end

return CReceiveSummerReward

