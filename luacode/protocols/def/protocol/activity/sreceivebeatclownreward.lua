-- Decompiled using luadec 2.2 rev: 895d923 for Lua 5.3 from https://github.com/viruscamp/luadec
-- Command line: -se UTF8 luacode/protocols/def/protocol/activity/sreceivebeatclownreward.lua 

-- params : ...
-- function num : 0 , upvalues : _ENV
local ProtocolBufferStaticFunctions = ((CS.PixelNeko).Net).ProtocolBufferStaticFunctions
local SReceiveBeatClownReward = dataclass("SReceiveBeatClownReward", require("framework.net.protocol"))
SReceiveBeatClownReward.ProtocolType = 2618
SReceiveBeatClownReward.MaxSize = 65535
SReceiveBeatClownReward.times = 0
SReceiveBeatClownReward.Ctor = function(self, client)
  -- function num : 0_0 , upvalues : SReceiveBeatClownReward
  ((SReceiveBeatClownReward.super).Ctor)(self, client)
end

SReceiveBeatClownReward.Marshal = function(self, buffer)
  -- function num : 0_1 , upvalues : ProtocolBufferStaticFunctions
  if not (ProtocolBufferStaticFunctions.WriteInt32)(buffer, self.times) then
    return false
  end
  return true
end

SReceiveBeatClownReward.Unmarshal = function(self, buffer)
  -- function num : 0_2 , upvalues : ProtocolBufferStaticFunctions
  local ret = true
  ret = (ProtocolBufferStaticFunctions.ReadInt32)(buffer)
  if not ret then
    return ret
  end
  return ret
end

return SReceiveBeatClownReward

