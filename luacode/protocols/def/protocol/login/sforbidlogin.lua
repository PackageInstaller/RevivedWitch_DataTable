-- Decompiled using luadec 2.2 rev: 895d923 for Lua 5.3 from https://github.com/viruscamp/luadec
-- Command line: -se UTF8 luacode/protocols/def/protocol/login/sforbidlogin.lua 

-- params : ...
-- function num : 0 , upvalues : _ENV
local ProtocolBufferStaticFunctions = ((CS.PixelNeko).Net).ProtocolBufferStaticFunctions
local SForbidLogin = dataclass("SForbidLogin", require("framework.net.protocol"))
SForbidLogin.ProtocolType = 1083
SForbidLogin.MaxSize = 65535
SForbidLogin.restTime = 0
SForbidLogin.Ctor = function(self, client)
  -- function num : 0_0 , upvalues : SForbidLogin
  ((SForbidLogin.super).Ctor)(self, client)
end

SForbidLogin.Marshal = function(self, buffer)
  -- function num : 0_1 , upvalues : ProtocolBufferStaticFunctions
  if not (ProtocolBufferStaticFunctions.WriteInt64)(buffer, self.restTime) then
    return false
  end
  return true
end

SForbidLogin.Unmarshal = function(self, buffer)
  -- function num : 0_2 , upvalues : ProtocolBufferStaticFunctions
  local ret = true
  ret = (ProtocolBufferStaticFunctions.ReadInt64)(buffer)
  if not ret then
    return ret
  end
  return ret
end

return SForbidLogin

